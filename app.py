...
with st.sidebar:
    ...
    if weight_files:
        st.caption(f"Backbone checkpoints available: {len(weight_files)}")
    else:
        st.caption("No registered backbone checkpoint found in current folder.")

if page == "Task 2: Cross-Attention":
    import app_task2 as task2_ui
    ...
    st.markdown(
        "<p class='ve-subtitle'>Browse saved heatmaps and generate token-wise/overall overlays for your own image-text pairs.</p>",
        unsafe_allow_html=True,
    )
    ...
    with tab_saved:
        ...
        with c1:
            artifact_dir = st.text_input("Artifact directory", value=task2_ui.DEFAULT_ARTIFACT_DIR)
            selected_csv = st.text_input("Selected examples CSV", value=task2_ui.DEFAULT_SELECTED)
        with c2:
            summary_path = st.text_input("Summary JSON", value=task2_ui.DEFAULT_SUMMARY)
            tensor_index_csv = st.text_input("Tensor index CSV", value=task2_ui.DEFAULT_TENSOR_INDEX)
        task2_ui.render_saved_heatmaps_tab(
    ...
    with tab_custom:
        summary_path_custom = st.text_input(
            "Summary JSON (for default config)",
            value=task2_ui.DEFAULT_SUMMARY,
            key="task2_summary_custom_root",
        )
        task2_ui.render_custom_heatmap_tab(
            summary_path=summary_path_custom,
            checkpoints=weight_files if weight_files else None,
        )
    st.stop()

if page == "Task 4: Prompt Transfer":
    import app_task4 as task4_ui
    ...
    st.markdown(
        "<p class='ve-subtitle'>Review prompt-engineering results and run prompt-template analysis on your own inputs.</p>",
        unsafe_allow_html=True,
    )

    adapters = task4_ui._list_adapter_weights()
    tab_report, tab_playground = st.tabs(["Prompt Analysis Report", "Custom Prompt Playground"])

    with tab_report:
        c1, c2, c3 = st.columns([1.2, 1.2, 1], gap="large")
        with c1:
            results_path = st.text_input("Results CSV", value=task4_ui.DEFAULT_RESULTS)
            diagnostics_path = st.text_input("Diagnostics CSV", value=task4_ui.DEFAULT_DIAGNOSTICS)
        with c2:
            summary_path = st.text_input("Run summary JSON", value=task4_ui.DEFAULT_SUMMARY)
            prompt_guide_path = st.text_input("Prompt guide MD", value=task4_ui.DEFAULT_PROMPT_GUIDE)
        with c3:
            failure_report_path = st.text_input("Failure report MD", value=task4_ui.DEFAULT_FAILURE_REPORT)
        task4_ui.render_analysis_report(
    ...
    with tab_playground:
        task4_ui.render_custom_playground(
            checkpoints=weight_files,
            adapters=adapters,
        )
    st.stop()

if page == "Task 5: Robustness":
    import app_task5 as task5_ui
    ...
    st.markdown(
        "<p class='ve-subtitle'>Inspect robustness curves from your experiment and test FGSM/PGD + text adversaries on custom inputs.</p>",
        unsafe_allow_html=True,
    )
    ...
    with tab_report:
        ...
        with c1:
            artifact_dir = st.text_input("Artifacts folder", value=task5_ui.DEFAULT_ARTIFACT_DIR)
        with c2:
            if weight_files:
                default_idx = weight_files.index(task5_ui.DEFAULT_CHECKPOINT) if task5_ui.DEFAULT_CHECKPOINT in weight_files else 0
                report_checkpoint = st.selectbox(
                    "Checkpoint for text-curve eval",
                    weight_files,
                    index=default_idx,
                )
            else:
                report_checkpoint = None
                st.warning("No registered backbone checkpoints found for report-side text curve generation.")
        task5_ui._render_robustness_report(artifact_dir, report_checkpoint)

    with tab_custom:
        task5_ui._render_custom_test(weight_files)
    st.stop()
